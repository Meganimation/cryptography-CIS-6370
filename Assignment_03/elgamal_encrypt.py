#create a private and public key pair, encrypt a message with the public key, and decrypt it with the private key using ElGamal encryption
...
def main():
    ...
    if public_key is None:
        ...
        print("Loaded public key and ciphertext from file.")
        view_choice = questionary.select(
            "Would you like to view the previously generated public key and ciphertext before proceeding?",
            choices=["Yes", "No"]
        ).ask()
        if view_choice == "Yes":
            print('************************************')
    ...
    choice = questionary.select("Would you like to encrypt or decrypt?", choices=["Encrypt", "Decrypt", os.path.exists(KEY_FILE) and "Generate New Private Key"]).ask()
    if choice == 'Encrypt':
        if (public_key):
